...
@app.command()
def get_topics(github_handle: Annotated[str, typer.Argument()]) -> dict:
    formatted_github_handle = format_github_handle(github_handle)
    typer.echo(f"Fetching your topics at {formatted_github_handle}...")
    topics = get_topics_frequencies(github_handle=formatted_github_handle)
    typer.echo(topics)
    return topics
...
def get_topics_pages(
    ...
    topics = get_topics(github_handle=github_handle)
    ...
    for topic, frequency in topics:
        ...
        if topic != "github-actions":
            continue
        result: list[TopicProject] = get_topic_projects(topic=topic, max_pages=10)
