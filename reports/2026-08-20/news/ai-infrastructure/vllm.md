# vLLM Project News — 2026-08-20

> Public GitHub release and roadmap evidence. Upcoming items are indications, not commitments.

Repository: [vllm-project/vllm](https://github.com/vllm-project/vllm)

## Latest stable: [v0.27.1](https://github.com/vllm-project/vllm/releases/tag/v0.27.1)

- Tag: `v0.27.1`
- Published: 2026-08-11T10:47:49Z
- Support quantized DSpark Markov heads (#50424)

## Publicly indicated upcoming work

- **Milestone** [v0.28.0 cherry picks](https://github.com/vllm-project/vllm/milestone/35)

## Hacker News discussions

No matching current Hacker News discussion was found.
